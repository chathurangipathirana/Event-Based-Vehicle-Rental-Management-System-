...
/** Send booking assignment details to the assigned driver. */
function emailDriverBookingAssignment(PDO $pdo, int $bookingId): ?bool
{
    $booking = fetchBookingNotificationContext($pdo, $bookingId);
    ...
    $bookingNumber = htmlspecialchars($booking['booking_number'] ?? '', ENT_QUOTES, 'UTF-8');
    $eventDate = htmlspecialchars($booking['event_date'] ?? '', ENT_QUOTES, 'UTF-8');
    $subject = 'New booking assignment ' . ($booking['booking_number'] ?? '') . ' - ' . ($booking['event_date'] ?? '');
    $assignmentDetails = buildDriverAssignmentDetailsHtml($booking);

    $message = "<html><body style=\"font-family: Arial, sans-serif; color: #1e293b; line-height: 1.6;\">"
        . "<p>Dear {$driverName},</p>"
        . "<p>You have been assigned to booking <strong>{$bookingNumber}</strong> on <strong>{$eventDate}</strong>.</p>"
        . "<p>Please review the trip details below and be ready at the pickup location on time.</p>"
        . $assignmentDetails
        . "<p>If you have any questions, contact the FleetElite operations team.</p>"
        . "<p>Thank you,<br>FleetElite Vehicle Rental</p>"
        . "</body></html>";

    return sendHtmlEmail($email, $subject, $message, $booking['driver_name'] ?? null);
}

/** Send customer and driver notifications after booking approval. */
function sendBookingApprovalNotifications(PDO $pdo, int $bookingId, ?array $invoice = null): array
{
    if ($invoice === null) {
        $invoice = getOrCreateBookingInvoice($pdo, $bookingId);
    }

    return [
        'customer' => emailBookingInvoice($pdo, $invoice),
        'driver' => emailDriverBookingAssignment($pdo, $bookingId),
    ];
}
...
function formatApprovalNotificationMessage(array $results): string
{
    ...
    if ($customerSent && $driverResult === true) {
        return 'Booking approved. Confirmation email sent to the customer and assignment details sent to the driver.';
    }

    if ($customerSent && $driverResult === null) {
        return 'Booking approved and confirmation email sent to the customer.';
    }

    if ($customerSent && $driverResult === false) {
        return 'Booking approved and customer notified, but the driver assignment email could not be sent. Please verify the driver email address.';
    }

    if (!$customerSent && $driverResult === true) {
        return 'Booking approved and driver notified, but the customer confirmation email could not be sent. Please verify the customer email address.';
    }

    if (!$customerSent && $driverResult === false) {
        return 'Booking approved, but both customer and driver emails could not be sent. Please verify the email addresses.';
    }

    if (!$customerSent && $driverResult === null) {
        return 'Booking approved and invoice created, but the confirmation email could not be sent. Please verify the customer email address.';
    }

    return 'Booking approved, but notification emails could not be sent. Please verify the email addresses.';
}
